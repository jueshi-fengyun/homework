...
func (n *node) findNodeMdls(seg string) ([]*node, []Middleware) {
	var mdls = make([]Middleware, 0)
	var chaidNodes = make([]*node, 0)

	//需要判断正则匹配
	if n.path == "*" && n.starChild != nil {
		mdls = append(mdls, n.starChild.mdls...)
		chaidNodes = append(chaidNodes, n.starChild)
	}

	//需要解析参数路径
	if n.paramChild != nil {
		mdls = append(mdls, n.paramChild.mdls...)
		chaidNodes = append(chaidNodes, n.paramChild)
	}
	if n.children != nil {
		if _, ok := n.children[seg]; ok {
			chaidNodes = append(chaidNodes, n.children[seg])
		}
	}

	return chaidNodes, mdls
}
...
func (r *router) findMdls(root *node, segs []string) []Middleware {
	...
	for _, seg := range segs {
		for _, cur := range queue {
			...
			child, mdlsTemp := cur.findNodeMdls(seg)
			queue = append(queue, child...)
			mdls = append(mdls, mdlsTemp...)
		}
	}
	return mdls
}
